...
exports.printMain = exports.printMenuItems = exports.initLogo = void 0;
...
let img;
const initLogo = () => new Promise((resolve, reject) => {
    (0, canvas_1.loadImage)(buffer)
        .then((t) => {
        img = t;
        resolve("Success");
    })
...
exports.initLogo = initLogo;
async function printMenuItems(pairs) {
    const canvas = (0, canvas_1.createCanvas)(760, 400);
    ...
    ctx.fillStyle = "#fff";
    ctx.fillText('Search results for "Wif"', 10, 50);
    pairs.forEach((p, index) => {
        drawRow(ctx, index + 1, p.baseToken.symbol, p.quoteToken.symbol, p.priceUsd, 80, p.baseToken.name);
    });
    const filePath = path_1.default.resolve(process.cwd(), `${Math.random().toString().slice(-15)}_${Date.now()}.png`);
...
exports.printMenuItems = printMenuItems;
function drawRow(ctx, index, baseToken, quoteToken, price, baseVertical = 80, baseTokenFull) {
    ...
    ctx.font = "400 14px Poppins";
    ctx.fillStyle = "#808080";
    ctx.fillText(`/${quoteToken.toUpperCase()}`, 50 + ctx.measureText(baseToken.toUpperCase()).width, y);
    ctx.font = "400 14px Poppins";
    ctx.fillStyle = "#fff";
    ctx.fillText(baseTokenFull, 80 +
        ctx.measureText(baseToken.toUpperCase()).width +
        ctx.measureText(`/${quoteToken.toUpperCase()}`).width, y);
    ctx.font = "400 14px Poppins";
    ctx.fillStyle = "#00BFFF";
    ctx.fillText(`${price}`, 100 +
        ctx.measureText(baseToken.toUpperCase()).width +
        ctx.measureText(`/${quoteToken.toUpperCase()}`).width +
        ctx.measureText(baseTokenFull).width, y);
}
const printMain = async (baseVertical = 80, label = "", subLabel = "", volume, price24h, price6h, price1h, liquidity, fdv) => {
    const canvas = (0, canvas_1.createCanvas)(760, 400);
    const ctx = canvas.getContext("2d");
    ...
    price24h = new Intl.NumberFormat().format(price24h) + "%";
    price6h = new Intl.NumberFormat().format(price6h) + "%";
    price1h = new Intl.NumberFormat().format(price1h) + "%";
